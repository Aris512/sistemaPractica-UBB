...
import type { Asignatura, CreateAsignaturaDTO } from "./types";

interface CreateModalProps {
  isOpen: boolean;
  onClose: () => void;
  onCreate?: (newAsignatura: Asignatura) => void;
}
...
export function CreateModal({ isOpen, onClose, onCreate }: CreateModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: CreateAsignaturaDTO = {
      nombre: nombre.trim(),
      ...
      <DialogContent className="sm:max-w-md bg-white border border-slate-200">
        <DialogHeader>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <DialogTitle className="text-slate-900 font-bold">Nueva Asignatura</DialogTitle>
              <DialogDescription className="text-slate-500 text-xs">
                Ingresa los datos para registrar una nueva asignatura en el sistema.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
