...
export function PerfilProfesor({ user, onBack }: PerfilProfesorProps) {
  const rolDisplay = (user.roles && user.roles[0]) || user.rol || "Tutor de Práctica / Profesor Colaborador";
  const fullName = `${user.nombre || ""} ${user.apellido || ""}`.trim().toUpperCase() || "DOCENTE TUTOR";
  const [especialidad, setEspecialidad] = useState<string>("Pedagogía y Gestión de Aula en Terreno");

  useEffect(() => {
    if (!user?.rut) return;

    // Intentar consultar datos adicionales si existen en el backend
    fetch(`http://localhost:8080/api/profesores/rut/${encodeURIComponent(user.rut)}`)
      ...
      .then((data) => {
        if (data?.asignatura?.nombre) {
          setEspecialidad(data.asignatura.nombre);
        }
      })
      .catch(() => {});
  }, [user?.rut]);
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto py-2">
      ...
      {/* Tarjeta 1: Detalles académicos / docentes / supervisión */}
      <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
        <div className="p-6 sm:p-8">
          <div className="flex items-center justify-between gap-4 mb-2">
            <h2 className="text-base sm:text-lg font-bold text-slate-900">
              Detalles Académicos y de Supervisión de Prácticas
            </h2>
          ...
          <p className="text-sm sm:text-base font-semibold text-slate-900 mb-6">
            Departamento de Ciencias de la Educación • Red de Centros de Práctica UBB
          </p>
            ...
            <div className="divide-y divide-slate-100">
              <div className="py-3.5 flex items-center justify-between gap-4">
                ...
                <span className="text-sm font-medium text-slate-800 text-right">
                  {rolDisplay.replace("ROLE_", "").replace("_", " ")}
                </span>
              </div>
              ...
              <div className="py-3.5 flex items-center justify-between gap-4">
                ...
                <span className="text-sm font-medium text-slate-800 text-right">
                  Pedagogías y Formación Docente
                </span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 divide-y md:divide-y-0 divide-slate-100">
            ...
            <div className="divide-y divide-slate-100">
              ...
              <div className="py-3.5 flex items-center justify-between gap-4">
                ...
                <span className="text-sm font-medium text-sky-800 text-right">
                  {rolDisplay}
                </span>
              </div>
            </div>
            ...
            <div className="divide-y divide-slate-100">
              <div className="py-3.5 flex items-center justify-between gap-4">
                ...
                <span className="text-sm font-medium text-slate-800 text-right truncate">
                  {user.correo || `${user.rut ? user.rut.replace(/[^0-9kK]/g, "") : "docente"}@ubiobio.cl`}
                </span>
              </div>
